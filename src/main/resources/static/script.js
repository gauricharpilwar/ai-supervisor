...
function showTab(name){ document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active', b.dataset.tab===name)); document.querySelectorAll('.tab').forEach(s=>s.classList.remove('show')); $(name).classList.add('show'); }

// -------------------- Events --------------------
document.querySelectorAll('nav button').forEach(b=>b.addEventListener('click', ()=>{ showTab(b.dataset.tab); if(b.dataset.tab==='learned') loadKB(); }));
...
function page(list, name){
  const start = (S.page[name]-1)*S.page.size;
  const items = list.slice(start, start+S.page.size);
  const pages = Math.max(1, Math.ceil(list.length/S.page.size));
  S.page[name]=Math.min(S.page[name], pages);
  return { items, pages };
}
function renderPager(elId, name, pages){
  const c=$(elId); c.innerHTML='';
  for(let i=1;i<=pages;i++){
    const b=document.createElement('button'); b.textContent=i; if(i===S.page[name]) b.disabled=true;
    b.onclick=()=>{ S.page[name]=i; render(); }; c.appendChild(b);
  }
}
...
function render(){
  ...
  // Pending
  const p = page(pend,'pending'); $('pendingTable').innerHTML = `
    <tr><th>ID</th><th>Question</th><th>Caller</th><th>Created</th><th>Status</th><th>Action</th></tr>
    ${p.items.map(r=>`
      <tr>
        <td>${r.id}</td>
        <td><a href="#" data-id="${r.id}" class="details">${text(r.question||'')}</a></td>
        <td>${text(r.callerName||'')}</td>
...
function wireDetails(tableId){
  document.querySelectorAll(`#${tableId} a.details`).forEach(a=>{
    a.onclick=(e)=>{
      ...
      $('modalBody').innerHTML=`
        <p><b>ID</b>: ${r.id}</p>
        <p><b>Caller</b>: ${text(r.callerName||'')}</p>
        <p><b>Question</b>: ${text(r.question||'')}</p>
...
async function unresolveReq(id){
  ...
  log(`Marked #${id} UNRESOLVED`);
  toast('Marked UNRESOLVED');
  await loadRequests();
}
